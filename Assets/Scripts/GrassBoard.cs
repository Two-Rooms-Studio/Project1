...
public class GrassBoard : Board {
	new public void init (int p_rows, int p_cols, GameObject tileObject)
	{
		//set up demo grass board
		base.init (p_rows, p_cols, tileObject);
		...
		for (int x = 0; x < cols; x++) {
			for (int y = 0; y < rows; y++) {
				...
				GameObject instance = Instantiate (tileObject, new Vector3 ((float)x + xPadding, (float)y + yPadding, 0.0f), Quaternion.identity, boardHolder);
